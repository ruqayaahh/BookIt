...
const loginUser = async (req, res, next) => {
  try {
    ...
    console.log(isNotDefined, "user login");
    // if any key is undefined, throw error
    if (isNotDefined.length) {
      let invalids = "";
      isNotDefined.forEach((key) => (invalids += " " + key));
...
const deleteUserBook = async (req, res, next) => {
  try {
    ...
    console.log(deletedBook.rows, "deletedBook");
    res.locals.deletedBook = deletedBook.rows[0];
    return next();
  } catch (error) {
    return next({
      log: "Error occured in deleteUserBook middleware",
  ...
  try {
    ...
    if (!in_custody)
      return next({
        log: "Error occured in lendUserBook middleware",
        status: 400,
        message: "Request body data undefined.",
      });
    const lentBook = await lendBookOut([book_id, user_id, in_custody]);
    console.log(lentBook.rows, "lentBook");
    res.locals.lentBook = lentBook.rows[0];
    return next();
  } catch (error) {
    return next({
      log: "Error occured in lendUserBook middleware",
